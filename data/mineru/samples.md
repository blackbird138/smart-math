...
vii 参考文献 687 符号索引 689 名词索引暨英译 691  
...
最后, 许多师友和读者对书稿提供了建议和指正, 无法一一备述, 记忆所及且有署名者包括: 曹连谦, 陈泽如, 褚浩云, 丁一文, 高剑伟, 蓝青, 马致远, 孙超超, 万铖睿, 王宇腾, 温焯晾, 吴家驹, 薛江维, 尹梓僮, 杨家铭, 朱奕 (按照姓名拼音排序). 在此致谢.  
...
所以只要取 $u$ 和 $\mathbf { \sigma } _ { v }$ 分别为 $\begin{array} { r } { - \frac { q } { 2 } + \frac { \sqrt { - 3 D } } { 2 \cdot 3 ^ { 2 } } } \end{array}$ 和 $\begin{array} { r } { - \frac { q } { 2 } - \frac { \sqrt { - 3 D } } { 2 \cdot 3 ^ { 2 } } } \end{array}$ 的立方根, 则 $u$ 和 $\mathbf { \sigma } _ { v }$ 都是辅助方程 (1.1.1) 的解. 我们断言:  
...
以上考虑的都是方程的复数解, 而在推导过程中论及平方根和立方根时, 也一律在复数系里操作, 否则无以穷尽 $X ^ { 3 } + p X + q = 0$ 的通解. 这就产生了一些耐人寻味的观察.  
...
沿用先前关于三次方程 $X ^ { 3 } + p X + q = 0$ 的符号, 记其三个复根为 $x _ { 1 } , x _ { 2 } , x _ { 3 }$ (计入重数). 在公式 (1.1.4) 中, 我们通过一个六次辅助方程的解 $u _ { 1 }$ , $u _ { 2 }$ , $u _ { 3 }$ , $v _ { 1 }$ , $v _ { 2 }$ , $\left| v _ { 3 } \right|$ 来表达 $\left| x _ { 1 } \right|$ $\mathbf { \Phi } _ { 1 } , x _ { 2 } , x _ { 3 }$ . 反过来, $x _ { 1 }$ , $x _ { 2 }$ , $x _ { 3 }$ 也能表达辅助方程的根: 适当地重排 $u _ { 1 } , u _ { 2 } , u _ { 3 }$ 后, 不妨假定  
...
![](images/d9cb3372ba7707ae55df4d440f2094f55236f011ff870cf36887f6622b0ac670.jpg)  
...
进一步, $\mathbb { Z } [ \mathrm { i } ]$ 对复共轭运算 $x + \mathrm { i } y \mapsto x - \mathrm { i } y$ 也保持封闭. 形如 $x + \mathrm { i } y$ 的复数 $( x , y \in \mathbb { Z } )$ 也称为 Gauss 整数.  
...
![](images/c7d92e6701eeaabfddd9ac945d32a761dd49a225a53773b3e1698ea50b043454.jpg)  
图 1-2  

综上, 我们通过 $Q  t$ 建立一一对应  

$$
\begin{array}{c} \{ \begin{array} { l } { ( a , b ) ~  ~ a ^ { 2 } + b ^ { 2 } = 1 , ~ a , b \in \mathbb { Q } } \\ { ( a , b ) \neq ( 1 , 0 ) } \end{array} \} \overset { \overset { \mathrm { \scriptscriptstyle 1 : 1 } } { \longleftrightarrow } } { \longleftrightarrow } \mathbb { Q } .  \end{array}
$$  

对应的左侧欠缺美感, 因为它排除了 $P = ( 1 , 0 )$ . 解决方法倒也简单: 另外容许参数 $\mathbf { \Psi } _ { t }$ 为 $\propto$ , 仅作为一个符号来理解, 而相应的直线 $\ell$ 是过 $P$ 的切线; 合理地设想切线在 $P$ 点交单位圆两次, 对应的解自然当是 $Q = P$ 了.  
...
没有满足 $X Y Z \neq 0$ 的整数解; 借由通分, 等价的说法是此方程没有满足 $X Y Z \neq 0$ 的有理数解.  
...
$$
S : = \left\{ ( x , y , z ) \in \mathbb { Z } _ { \geq 1 } : x ^ { 2 } + 4 y z = p \right\} .
$$  

(i) 考虑映射 $f : S  S$ 如下  

$$
f ( x , y , z ) = \left\{ { \begin{array} { l } { ( x + 2 z , z , y - x - z ) , } \\ { ( 2 y - x , y , x - y + z ) , } \\ { ( x - 2 y , x - y + z , y ) , } \end{array} } \right.
$$  

验证此定义是合理的, 而且对所有 $( x , y , z ) \in S$ 皆有 $f ( f ( x , y , z ) ) = ( x , y , z )$ .  

(ii) 说明 $f : S  S$ 有唯一的不动点 $( x , y , z )$ , 亦即满足 $f ( x , y , z ) = ( x , y , z )$ 的点. 由此说明 $S$ 的元素个数是奇数.  

(iii) 按 $g ( x , y , z ) ~ = ~ ( x , z , y )$ 定义映射 $g : S  S$ , 说明 $g$ 有不动点. 以此说明$x ^ { 2 } + y ^ { 2 } = p$ 有整数解.  

线性方程组 相对于不定方程和多元高次方程组, 求解线性方程组要简单得多. 线性方程意指一次方程. 线性方程组是形如  

$$
\left\{ \begin{array} { c } { a _ { 1 1 } X _ { 1 } + \cdot \cdot \cdot + a _ { 1 n } X _ { n } = b _ { 1 } } \\ { a _ { 2 1 } X _ { 1 } + \cdot \cdot \cdot + a _ { 2 n } X _ { n } = b _ { 2 } } \\ { \vdots \qquad \vdots } \\ { a _ { m 1 } X _ { 1 } + \cdot \cdot \cdot + a _ { m n } X _ { n } = b _ { m } } \end{array} \right.
$$  

的方程组, 其中 $a _ { i j }$ 和 $b _ { i }$ 是给定的常数 $( 1 \leq i \leq n$ , $1 \leq j \leq m$ ), 而 $X _ { 1 } , \ldots , X _ { n }$ 是变元.  
这样的方程可以在 $\mathbb { Q }$ , $\mathbb { R }$ 或 $\mathbb { C }$ 上来考虑, 我们暂且不去明确所用的数系.  

根本的问题是: 如何判定方程组是否有解? 若有解, 如何高效地求解? 所有解$( X _ { 1 } , \ldots , X _ { n } )$ 构成的集合 (顺理成章地称为解集) 有怎样的结构?  
...
$$
X _ { 1 } = { \frac { \left| b _ { 1 } \quad a _ { 1 2 } \right| } { \left| a _ { 1 1 } \quad a _ { 1 2 } \right| } } , \quad X _ { 2 } = { \frac { \left| a _ { 1 1 } \quad b _ { 1 } \right| } { \left| a _ { 2 1 } \quad a _ { 2 2 } \right| } } .
$$  

依然设 $n = m$ ; 对于一般的 $\mathbf { \Omega } _ { m }$ 的情形, 我们也有称为 Cramer 法则的行列式解法,详见 $\ S 5 . 7$ . 然而它涉及 $n + 1$ 个 $\mathbf { \Omega } _ { n }$ 阶行列式, 非但需要行列式的一般理论, 所需的计算量也随 $n$ 增长而暴增; Cramer 法则的用处在于理论层面, 不在计算层面. 我们行将介绍的 Gauss–Jordan 消元法则提供了一个简单快速的求解手段.  

小结 现在进一步解释早先提出的问题: 何谓代数?  

$\star$ 何谓方程 — 来自经过有限次的加, 减, 乘, 除 (分母非零) 四则运算得到的表达式.$\star$ 何谓数 — 至少包括 $\mathbb { Q }$ , $\mathbb { R }$ , $\mathbb { C }$ 这些常用的数系; 它们都具备四则运算, 但除法要求分母非零. 注意到 $\mathbb { Z }$ 不在列表中, 因为除法在 $\mathbb { Z }$ 中不能通行无阻.$\star$ 何谓求解的技艺 — 判定方程是否有解, 如何精确求解, 给出一套尽量高效的算法,或者至少给出逼近方程的解的手段. 算法是一切应用的核心.  

根据关于不定方程的讨论, 可知方程解集的性状和 “数” 的界定密切相关, 采用的技术也随之而千变万化.  

我们关心的还有解集的结构. 数学中所谓的 “结构” 难以三言两语说清. 结构的一个重要面向是对称性. 在几何的经典场景中, 对称性意指图像在一族刚体变换作业下的不变性; 这里所谓的刚体变换包括平移, 旋转, 镜射, 后续章节将有完整讨论. 对称性也是美感经验的一大要素, 人类对于对称似乎有先天的敏感和爱好, 数学工作者尤其如此.  

$\star$ 方程 $X ^ { 2 } + Y ^ { 2 } = 1$ 在平面 $\mathbb { R } ^ { 2 }$ 上的解集是单位圆, 对任何保持圆心不动的刚体变换都保持对称, 这些变换包括绕圆心的任意转动, 以及相对于 $X$ 轴或 $Y$ 轴的镜射等.  

$\star$ 考虑线性方程组 $X + Y = 1$ , 具体起见, 仍在平面 $\mathbb { R } ^ { 2 }$ 上求解. 其解集无非是直线:  

![](images/618b885e1ed93ff563c31754d0ef9f4e11f7dc38691b36a2c50c2714b24218fd.jpg)  
图 1-3  

直线上的点对于沿着 $( - 1 , 1 )$ 方向的所有平移都保持不变, 此外, 它相对于直线$X = Y$ (上图虚线) 的镜射也具有对称性.  

$\star$ 除了基于图像的对称性, 还有针对抽象对象 (例如代数中使用的变元) 或其间的关系的对称性. 这种对称有时涉及特定集合或属性在对象的任意重排之下, 或在按一定顺序轮换之下的不变性, 但也可以涉及更广的变换. 这类对称性在三次方程的讨论中已经初露端倪.  

上面给出了代数方程的几种典型例子, 那么何谓非代数方程? 典型的例子是涉及极限, 例如涉及无穷级数的操作, 这类问题是数学分析的主场. 另一类是涉及不等式, 例如形如  

$$
\left\{ \begin{array} { l } { a _ { 1 1 } X _ { 1 } + \cdot \cdot \cdot + a _ { 1 n } X _ { n } \leq b _ { 1 } } \\ { a _ { 2 1 } X _ { 1 } + \cdot \cdot \cdot + a _ { 2 n } X _ { n } \leq b _ { 2 } } \\ { \qquad \quad \vdots \qquad \vdots } \\ { a _ { m 1 } X _ { 1 } + \cdot \cdot \cdot + a _ { m n } X _ { n } \leq b _ { m } } \end{array} \right.
$$  

的方程组; 这里取 $a _ { i j }$ 和 $b _ { i }$ 为实数, 所求的解 $X _ { 1 } , \ldots , X _ { n }$ 亦然, 这是因为 $\mathbb { R }$ 相对于 $\mathbb { C }$ 具有额外的 “序结构”: 任两个实数可以合理地比大小.  

涉及不等式的方程组也称为半代数的. 中学数学学过的线性规划便涉及这类方程组. 以 $n = 2$ 情形为例, 解空间一般是由有限多条直线围出的区域, 譬如  

![](images/704230f086a7435f14af34df139a8b4becacbebe32d3f0c8b3961de7d47cf810.jpg)  
图 1-4  

的形式. 这类解空间也有特殊的 “结构”: 至少在有界的情形下, 它们是多边形 (高维情形: 多面体). 线性规划所求的是形如 $c _ { 1 } X _ { 1 } + \cdots + c _ { n } X _ { n }$ 的函数在解空间上的极值. 当 $n = 2$ 时可以画图求解, $n = 3$ 时也可以发挥想象, 但实际应用场景中的 $n$ 成千上万. 如何将低维的几何直觉可靠地推广到一般维数的线性规划? 尽管这是一个半代数的问题, 线性方法对此仍不可或缺.  

若在线性规划的问题中另外要求 $X _ { 1 } , \ldots , X _ { n }$ 为整数, 对应的便是整数线性规划问题. 一如不定方程比求方程的复数解困难, 整数线性规划的难度也远高于线性规划. 按算法的术语说, 它是一个 NP 完全问题.  
...
除非另外声明, 对于变元个数 $n$ 和方程个数 $\mid m \mid$ 不加任何限制, 仅要求它们有限. 为了讨论方便, 且先假设是在 $\mathbb { C }$ 中求解.  

线性方程组及其衍生结构将占据本书大半篇幅. 之所以选择它们作为学习代数的踏脚石, 大致可以点出几个原因:  

$\star$ 在所有代数方程组中, 线性方程组是充分广泛, 同时又相对简单的一类例子.  
$\star$ 实际应用中, 线性方程组常见而且重要.  
$\star$ 对于更复杂的方程和进阶的代数研究, 线性方法往往是必要的工具, 而线性方程组本身又自然地涉及高次多项式与抽象代数结构的思想. 两方面的技术因而是相互交融的.  
$\star$ 最后, 线性方程组的具体求解过程很能体现数学的算法特性.  

既然拈出了算法作为一个必要, 有用而且可供操作的面向, 现在便来介绍称为Gauss–Jordan 消元法的求解手段. 大而化之地说, 消元法的思路是在同解的方程组之间过渡, 直至方程组化为一类可直接求解的形式为止. 何谓同解?  

定义 1.3.1 如果以 $X _ { 1 } , \ldots , X _ { n }$ 为变元的两个线性方程组有相同的解集, 则称它们是同解的.  

且先看消元法的一个简单特例.  

例 1.3.2 令 $\mathbf { \Phi } _ { a , b , c }$ 为给定的常数. 考虑  

$$
\left\{ \begin{array} { r c l c r c l } { { X _ { 1 } } } & { { - X _ { 2 } } } & { { + X _ { 3 } } } & { { = } } & { { a } } & { { \textcircled { 1 } } } \\ { { X _ { 1 } } } & { { - X _ { 2 } } } & { { - X _ { 3 } } } & { { = } } & { { b } } & { { \textcircled { 2 } } } \\ { { 2 X _ { 1 } } } & { { - 2 X _ { 2 } } } & { { - X _ { 3 } } } & { { = } } & { { c } } & { { \textcircled { 3 } } } \end{array} \right.
$$  

右列是方程的编号, 或谓 “行号”. 将第一个方程两边乘以 $^ { - 1 }$ 加到第二个方程; 类似地,将第一个方程两边乘以 $^ { - 2 }$ 加到第三个方程, 如是得到新的方程组  

$$
\left\{ \begin{array} { r r r r r } { { X _ { 1 } \quad - X _ { 2 } \quad } } & { { + X _ { 3 } } } & { { = } } & { { a } } & { { \qquad \textcircled { 1 } } } \\ { { } } & { { - 2 X _ { 3 } } } & { { = } } & { { b - a } } & { { \qquad \textcircled { 2 } ^ { \prime } : = \textcircled { 2 } - \textcircled { 1 } } } \\ { { } } & { { - 3 X _ { 3 } } } & { { = } } & { { c - 2 a } } & { { \qquad \textcircled { 3 } ^ { \prime } : = \textcircled { 3 } - 2 \cdot \textcircled { 1 } } } \end{array} \right.
$$  

我们想反解 $X _ { 3 }$ , 所以将 $\textcircled{2}$ 乘以 $- \frac { 1 } { 2 }$ , 然后用它消掉 $\textcircled{3} ^ { \prime }$ 的 $X _ { 3 }$ . 产物是  

$$
\left\{ \begin{array} { r r r r r } { { X _ { 1 } } \quad - { X _ { 2 } } \quad + { X _ { 3 } } } & { { = } } & { { a } } & { { \qquad \textcircled { 1 } } } \\ { { { } } } & { { { } } } & { { { X _ { 3 } } } } & { { = } } & { { { \textstyle \frac { a - b } { 2 } } } } & { { \qquad \textcircled { 2 } ^ { \prime \prime } : = - { \textstyle \frac { 1 } { 2 } } \textcircled { 2 } ^ { \prime } } } \\ { { 0 } } & { { = } } & { { { \textstyle \frac { - a - 3 b + 2 c } { 2 } } } } & { { \qquad \textcircled { 3 } ^ { \prime \prime } : = \textcircled { 3 } ^ { \prime } + 3 \cdot \textcircled { 2 } ^ { \prime \prime } } } \end{array} \right.
$$  

每一步都给出同解的方程组. 解集于是明朗了: 由下往上地解方程, 得到$\star$ 如果 $- a - 3 b + 2 c \neq 0$ , 则方程无解, 因为 $\textcircled{3} ^ { \prime \prime }$ 将是自相矛盾的;  

$\star$ 设 $- a - 3 b + 2 c = 0$ , 则 $\textcircled{2} ^ { \prime \prime }$ 给出 $X _ { 3 }$ , 代入 $\textcircled{1}$ , 得出方程的通解  

$$
\left\{ \begin{array} { l l } { \displaystyle X _ { 1 } = a + X _ { 2 } - X _ { 3 } } \\ { \displaystyle \quad = \frac { a + b } { 2 } + X _ { 2 } , } \\ { \displaystyle X _ { 3 } = \frac { a - b } { 2 } . } \end{array} \right.
$$  

注意到 $X _ { 2 }$ 在通解中是自由变元, 它不受约束, 可以任取.  

如果一切都取为实数, 将解集在三维空间中绘制, 则它或者是空集 (当 $- a - 3 b + 2 c \neq 0 )$ ,或者是落在平面 $\begin{array} { r } { X _ { 3 } = \frac { a - b } { 2 } } \end{array}$ 上的一条直线, 由坐标 $X _ { 2 }$ 参数化. 套用物理学的术语, 后一情形下可以说解集的自由度为 1, 因为它有一个可变参数.  

这里的自由度只是一个权宜说词, 随着向量空间理论的渐次铺展, 对此将有更加精确的界定.  

回到一般的线性方程组. 形如 (1.3.1) 的写法现在显得有些累赘了, 不如引进较为紧凑的符号.  

定义 1.3.3 我们将 (1.3.1) 的方程组以称为矩阵的方式记为  

$$
\left( \begin{array} { c c c c c } { a _ { 1 1 } } & { a _ { 1 2 } } & { \cdots } & { a _ { 1 n } } & { b _ { 1 } } \\ { a _ { 2 1 } } & { a _ { 2 2 } } & { \cdots } & { a _ { 2 n } } & { b _ { 2 } } \\ { \vdots } & { } & { } & { } & { \vdots } \\ { a _ { m 1 } } & { a _ { m 2 } } & { \cdots } & { a _ { m n } } & { b _ { m } } \end{array} \right)
$$  

的形式. 去掉最右一列得到的矩阵  

$$
\left( \begin{array} { c c c c } { a _ { 1 1 } } & { a _ { 1 2 } } & { \cdots } & { a _ { 1 n } } \\ { a _ { 2 1 } } & { a _ { 2 2 } } & { \cdots } & { a _ { 2 n } } \\ { \vdots } & { } & { } & { \vdots } \\ { a _ { m 1 } } & { a _ { m 2 } } & { \cdots } & { a _ { m n } } \end{array} \right)
$$  

称为该方程组的系数矩阵; 相对于此, 先前写下的带 $b _ { 1 } , \ldots , b _ { m }$ 的矩阵则称为增广矩阵.  

矩阵行, 列的具体记法是  

$$
\left( \begin{array} { c c c c c } & & & & & \\ & { \vdots } & & & \\ { \cdots } & { a _ { i j } } & { \cdots } & \\ & & & & { \sqrt { \frac { \hbar } { 2 } } } & \\ & & & & & \\ & & & & & { \sqrt { \frac { \hbar } { 2 } } } \end{array} \right) \xrightarrow [ ] { \ast \langle \boldsymbol { \mathrm { s } } \boldsymbol { \mathrm { t } } | }
$$  

每个矩阵元 $a _ { i j }$ 都等于 0 的矩阵称为零矩阵. 形如 $a _ { i i }$ 的矩阵元构成了 $A$ 的对角线1).  
为了排版考量, 有时也将 $a _ { i j }$ 写成 $\mathbf { \alpha } _ { a _ { i , j } }$ . 本书称 $\mathbf { \Omega } _ { m }$ 行 $\boldsymbol { n }$ 列的矩阵为 $m \times n$ 矩阵.  

由于方程组中的 $b _ { 1 } , \ldots , b _ { m }$ 角色毕竟不同于系数 $a _ { i j }$ , 有时在增广矩阵中作区隔:  

$$
( \begin{array} { c c c } { a _ { 1 1 } } & { \cdots } & { a _ { 1 n } } \\ { \vdots } & { } & { \vdots } \\ { a _ { m 1 } } & { \cdots } & { a _ { m n } } \end{array} ) \begin{array} { l } { b _ { 1 } } \\ { \vdots } \\ { b _ { m } } \end{array} ) .
$$  

Gauss–Jordan 消元法的思路是用以下三种操作来简化方程组, 或者换句话说, 简化相应的矩阵.  

(A) 设 $1 \leq i \neq k \leq m$ , 而 $\scriptstyle { c }$ 是任意常数. 将第 $\mathbf { \chi } _ { i }$ 行乘以 $\scriptstyle { c }$ 的结果加到第 $k$ 行, 其他的行保持不变:  

(B) 设 $1 \leq i < k \leq m$ . 交换第 $i$ 行和第 $k$ 行:  

$$
\begin{array} { r }  \mathbf { B } ( i , k ) : \begin{array} { c } { i \left( \begin{array} { c } { \vdots } \\ { \cdots \ a _ { i j } \ \cdots } \\ { \vdots } \\ { \cdots \ a _ { k j } \ \cdots } \\ { \vdots } \end{array} \right) } \end{array} \biggr \} _ { \mathcal { Z M } } \begin{array} { c } { \boxed { \begin{array} { c } { \vdots } \\ { \cdots \ a _ { k j } \ \cdots } \\ { \vdots } \\ { \cdots \ a _ { i j } \ \cdots } \\ { \vdots } \end{array} } } \\ { \end{array} } \end{array}
$$  

(C) 设 $1 \leq i \leq m$ 而 $\scriptstyle { c }$ 是非零常数. 将第 $\mathbf { \chi } _ { i }$ 行的每一项都乘以 $\scriptstyle { \begin{array} { l } { c } \end{array} }$ :  

$$
{ \bf C } ( i , c ) : \quad i \left( \begin{array} { c } { { \vdots } } \\ { { \cdots \ a _ { i j } \ \cdots } } \\ { { \vdots } } \end{array} \right) \curvearrow \cdots \quad \longmapsto \quad \left( \begin{array} { c } { { \vdots } } \\ { { \cdots \ c a _ { i j } \ \cdots } } \\ { { \vdots } } \end{array} \right)
$$  

这些操作称为对矩阵的初等行变换. 我们仅容许交换行的顺序, 列的顺序不变, 所以变元 $X _ { 1 } , \ldots , X _ { n }$ 的顺序恒定. 如果 $( x _ { 1 } , \ldots , x _ { n } )$ 是原矩阵对应的方程组的解, 则相对于初等行变换之后的矩阵, 它仍是对应的方程组的解.  

注意到上述每一种操作都可以被相应的逆操作撤销, 以回到原来的矩阵, 具体言之:  

$\star \mathrm { ~ A } ( i , k , c )$ 的逆操作是 $\mathrm { A } ( i , k , - c )$ ，$\star \mathrm { ~ B } ( i , k )$ 的逆操作是 $\mathrm { B } ( i , k )$ 自身,$\star \mathrm { ~ C ~ } ( i , c )$ 的逆操作是 $\mathrm { C } ( i , 1 / c )$ ,  

有请读者顺手检验. 综上:  

矩阵的初等行变换给出同解的方程组.  

也请注意到如果矩阵的某一列全为 0, 则无论如何作初等行变换, 该列依然为 0.  
...
$$
\left( \begin{array} { c c c c } { a _ { 1 1 } } & { a _ { 1 2 } } & { \cdots } & { a _ { 1 n } } \\ { a _ { 2 1 } } & { a _ { 2 2 } } & { \cdots } & { a _ { 2 n } } \\ { \vdots } & { } & { \ddots } & { \vdots } \\ { a _ { m 1 } } & { a _ { m 2 } } & { \cdots } & { a _ { m n } } \end{array} \right)
$$  

当它的形式如下所示时, 称之为行梯矩阵:  

$$
\left( \begin{array} { c c c } { { } } & { { } } & { { } } \\ { { } } & { { } } & { { } } \\ { { } } & { { } } & { { \vdots } } \end{array} \right)
$$  

其中左下空白部分的矩阵元皆为零, 涂灰的部分逐行向内严格缩进, 而且我们要求涂灰部分每一行的左端都是非零元, 它们称为此行梯矩阵的主元.  

更加严格却不尽直观的定义如下:  

$\star$ 存在整数 $0 \leq r \leq m$ 使得第 $\mathbf { \chi } _ { i }$ 行全为 0 当且仅当 $i > r$ (因此行梯矩阵中不全为0 的行恰好是前 $\boldsymbol { r }$ 行);  
$\star$ 对于每个 $1 \leq k \leq r$ (非零行的编号), 取  

$$
j _ { k } : = \operatorname* { m i n } \left\{ j : a _ { k j } \neq 0 \right\} ,
$$  

则 $j _ { 1 } < j _ { 2 } < \cdots < j _ { r }$ (相当于说涂灰部分逐行严格缩进).现前提及的主元无非是 a1,j1 , . . . , ar,jr .  

请读者沉思行梯矩阵的轮廓, 下述结果应该不言而喻  

练习 1.3.5 验证主元的个数 $r$ 满足 $0 \leq r \leq \operatorname* { m i n } \{ n , m \}$ . 无主元的行梯矩阵只能是零矩阵.  

定义 1.3.6 在关于行梯矩阵的定义中, 倘若进一步对所有 $1 \leq k \leq r$ 要求:  

$\star \ a _ { k , j _ { k } } = 1$ , 换言之, 主元全为 1;  

$\star \ i < k \ \Longrightarrow \ a _ { i , j _ { k } } = 0$ , 换言之, 落在主元以上的项全为 0;  

则称此矩阵为简化行梯矩阵.  

算法 1.3.7 (C. F. Gauss, W. Jordan) 对给定的矩阵如 (1.3.2), 按以下程序反复作初等行变换, 便能化之为行梯矩阵.  

(i) 如果矩阵的第一列全为 0, 则跳过第一列, 继续对下图框出的 “子矩阵” 作初等行变换:  

$$
( \begin{array} { l l l l } { { 0 [ a _ { 1 2 } } } & { { a _ { 1 3 } } } & { { \cdots } } & { { a _ { 1 n } } } \\ { { 0 [ a _ { 2 2 } } } & { { a _ { 2 3 } } } & { { \cdots } } & { { a _ { 2 n } } } \\ { { } } & { { \ddots } } & { { } } \\ { { 0 [ a _ { m 2 } } } & { { a _ { m 3 } } } & { { \cdots } } & { { a _ { m n } } ] } \end{array} ] ) ) ( \begin{array} { l } { { \begin{array} { l } { { \begin{array} { l } { { \begin{array} { l } { { \begin{array} { l } { \begin{ l } { \begin{array} { l } { \begin{array} { l } { \begin{ l } { \begin{array} { l } { \begin{ l } } { { \begin{ \begin{array} { l } { \begin{ l } } { \end{array} \begin{ l } } { \begin{ \begin{array} { l } { \begin{ l } } { \begin{ \begin{array} { l } { \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{array} { l } { \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ \begin{ l } } { \begin{ \begin{ l } } \begin{ } { \begin{ \begin{ l } } { \begin{ \begin{ l } } \begin{ } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ } \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ \begin{ l } } } { \begin{ \begin{ l } } { \begin{ \begin{ \begin{ l } } } { \begin{ \begin{ \begin{ l } } \begin{ } \begin{ l } } { \begin{ \begin{ \begin{ l } } { \begin{ \begin{ l } } } { \begin{ \begin{ \begin{ } \begin{ l } } { \begin{ \begin{ l } } } { \begin{ \begin{ \begin{ l } } } { \begin{ \begin{ [ } \begin{ l } } { \begin{ \begin{ l } } { \begin{ \begin{ l } } [ } { \begin{ [ } { [ \begin{ \Lambda } { \Lambda } { } { \Lambda } { \Lambda } { \Lambda } } { \Lambda { } { \Lambda } { \Lambda } { } { \Lambda } { \Lambda } { \Lambda } } { \Lambda } { \Lambda } { } { \Lambda } } { \Lambda } { \Lambda } { \Lambda } } { \end } \Lambda } { } \end } } { \Lambda } \end } } \end } } } } } } } } } \\ \end \end } } } \end } } } } } } \end } } } } } } \end{ } } } } } } \end{ } } } } } } } } } } } } \\ \end{ } } } } } } \end{ } } } }    \\ \end{ }    \end{array}      \end \end{array} \end{array} \end{array} \end{array} \end{array} \end{array} \end{array} \end{array} \end{array} \end{array}
$$  

全为 0 的列不受后续变换的影响, 可以放心舍去.  

(ii) 设矩阵第一列有非零元, 设之为 $\mathbf { \alpha } _ { a _ { k 1 } }$ . 进行先前标为 $\mathrm { B } ( k , 1 )$ 的变换以交换第 $k$ 行和第 1 行, 可以化到 $k = 1$ , 亦即 $a _ { 1 1 } \neq 0$ 的情形.  

(iii) 设 $a _ { 1 1 } \neq 0$ . 接着对每个 $1 < i \leq m$ , 进行先前标为 A $\begin{array} { r } { \left( 1 , i , - \frac { a _ { i 1 } } { a _ { 1 1 } } \right) } \end{array}$ 的变换, 将第一行乘以 $- \frac { a _ { i 1 } } { a _ { 1 1 } }$ 倍加到第 $i$ 行. 如此的效果是将 $a _ { 1 1 }$ 以下的矩阵元全变为 0. 于是矩阵进一步化为  

然后我们对框出的子矩阵继续操作.  

之所以将 $\mathbf { \alpha } _ { a _ { 1 1 } }$ 加黑, 是代表该矩阵元为主元. 而对于框出的子矩阵, 它或者处处是 0, 或者经过初等行变换还会给出新的主元. 依此类推, 算法必然在有限步内停止, 给出行梯矩阵.  

目前仅用到 (A), (B) 两种类型的运算. 为了从行梯矩阵进一步得到简化行梯矩阵,我们对给定的行梯矩阵继续以下操作, 这里需要 (C) 型操作.  

(iv) 对每个主元 $a _ { k , j _ { k } }$ , 作变换 $\mathrm { C } \left( k , a _ { k , j _ { k } } ^ { - 1 } \right)$ 以化约到行梯矩阵的主元全为 1 的情形.  

(v) 对每个主元(取值为1), 假设它位于第 $k$ 行上, 对每个 $i < k$ 作变换 $\mathrm { A } \left( k , i , - a _ { i j _ { k } } \right)$ ;换言之, 将第 $k$ 行乘以 $- a _ { i , j _ { k } }$ 加到第 $i$ 行上. 此操作将落在主元以上的矩阵元全化为 0.  

显然, 最后得到的矩阵必然是简化行梯矩阵.  

消元法的执行方式并不唯一. 比如步骤 2 涉及非零元 $\mathbf { \alpha } _ { a _ { k 1 } }$ 的选法, 而且我们在选定非零元 $\mathbf { \alpha } _ { a _ { k 1 } }$ 之前还可以进行若干次 (A) 或 (C) 型的初等行变换, 将各项化为更方便计算的形式. 当矩阵元全为整数时, 这种操作手法特别常见.  

同一个矩阵可以通过初等行变换过渡到种种不同的行梯矩阵. 相对于此, 初等行变换给出的简化行梯矩阵则是唯一确定的. 本章习题部分将勾勒其证明.  

练习 1.3.8 假定读者对于编程有一定程度的了解. 令 $N : = \operatorname* { m a x } \{ n , m \}$ . 试说明随着 $N$ 增大, Gauss–Jordan 消元法的涉及的操作次数的增长约略被 $N ^ { 3 }$ 的某个常数倍控制.  
...
$$
\left( \begin{array} { c c c } { a _ { 1 1 } } & { \cdots } & { a _ { 1 n } } \\ { \vdots } & { } & { \vdots } \\ { a _ { m 1 } } & { \cdots } & { a _ { m n } } \end{array} \right) \left( \begin{array} { c } { b _ { 1 } } \\ { \vdots } \\ { b _ { m } } \end{array} \right)
$$  

施行 Gauss–Jordan 消元法, 化之为简化行梯矩阵, 形如  

$$
\left( \begin{array} { c c c c c c } { { } } & { { } } & { { } } & { { } } & { { } } & { { } } \\ { { } } & { { } } & { { } } & { { } } & { { } } & { { } } \\ { { } } & { { } } & { { } } & { { } } & { { } } & { { } } \\ { { } } & { { } } & { { } } & { { } } & { { } } & { { } } \end{array} \right)
$$  
...
$$
\left( \begin{array} { l l l } { 0 } & { \ldots } & { 0 } \end{array} \Big | \begin{array} { l } { 1 } \end{array} \right)
$$  

的行, 或者换句话说, 方程组包含等式 $0 = 1$ , 则方程组无解.  

(ii) 假设没有如上形式的行. 记简化行梯矩阵主元的个数为 $\boldsymbol { r }$ , 将主元出现的列号依次排开, 记为  

$$
1 \leq j _ { 1 } < \cdot \cdot \cdot < j _ { r } \leq n ;
$$  

称它们对应的列为主列. 则矩阵的第 $k$ 行 $( 1 \leq k \leq r )$ 对应到方程  

$$
X _ { j _ { k } } + \sum _ { j > j _ { k } } a _ { k j } X _ { j } = b _ { k } ;
$$  

注意到简化行梯矩阵的定义确保 $X _ { j _ { k + 1 } } , \dotsc , X _ { j _ { r } }$ 在左式中的系数为 0. 由此立可反解每个主列所对应的变元  

$$
X _ { j _ { k } } = b _ { k } - \sum _ { j > j _ { k } \atop \exists \models \emptyset \mid \mathbf { \bar { f } } \mid \mathbf { \bar { f } } } a _ { k j } X _ { j } .
$$  

注意到方程组对非主列所对应的变元 $X _ { j }$ 没有约束 — 它们是 “自由变元”.  

(iii) 因此 $\mathbf { \Omega } _ { n }$ 元线性方程组 (1.3.1) 或者无解, 或者它的解集依赖于 $n - r$ 个自由变化的参数 (亦即其 “自由度” 为 $n - r )$ , 其中 $\textit { r }$ 是 Gauss–Jordan 消元法给出的主元个数.  

以上解方程 (1.3.1) 时对整个增广矩阵进行了消元. 包含 $b _ { 1 } , \ldots , b _ { m }$ 的增广列当然是重要的, 它关系到方程组是否有解. 但只要方程组有解, 主元就不可能出现在增广列,否则简化行梯矩阵将有形如 $( 0 \ \cdot \cdot \cdot \ 0 \mid 1 )$ 的行. 这些讨论表明: 一旦方程组 (1.3.1) 有解, 则增广矩阵的主元无非是系数矩阵的主元.  

尽管可以证明消元法给出的简化行梯矩阵是唯一的, 由于主元依赖于变元$X _ { 1 } , \ldots , X _ { n }$ 的排序, 主元相对于方程组本身仍显得是一个外部的, 不尽自然的概念. 另一方面, 上述讨论又表明一旦方程有解, 则主元个数 $\boldsymbol { r }$ 是一个内在于方程组本身的概念, 它连同变元个数 $n$ 一并决定了解集的自由度 $n - r$ . 为了理清这些问题, 有必要为线性方程组建立更深刻也更自然的理论框架. 这将涉及向量空间和线性变换的语言.  

对于 Gauss–Jordan 消元法, 另一则重要观察是它只涉及矩阵元的四则运算, 和我们具体选取的数系 $\mathbb { Q }$ , $\mathbb { R }$ , $\mathbb { C }$ 等并无关系. 然而若考虑系数在 $\mathbb { Z }$ 上的矩阵就会导致麻烦,因为 Gauss–Jordan 消元法涉及除法. 这就启发我们进一步放宽数系的概念, 转向容许四则运算的抽象代数结构, 称为域. 由于代数结构的严谨定义离不开集合与映射, 首务是扼要地介绍集合论的相关概念, 这是下一章的主题.  

# 习题  

1. 说明 (或回忆) 如何用尺规作图来实现两个线段长度的相加和相减. 在给定单位长度线段的前提下, 用尺规作图实现线段长度的乘法和除法. 这些事实表明, 只要指定单位长度, 则所有能从尺规作图得到的线段长度对四则运算封闭, 因此它们也构成某种数系, 或者更精确地说, 构成一个 “域”.请进一步说明 (或回忆) 如何用尺规作图构造平方根.  

2. 设 $\zeta \in \mathbb { C }$ 是 5 次单位原根, 亦即 $\zeta ^ { k } = 1 \iff 5 \mid k$ , 比如取 $\zeta = e ^ { 2 \pi \mathrm { i } / 5 }$ 即可. 设 $\textstyle a , b \in \mathbb { Q }$ ,定义  

$$
\alpha _ { i } = \zeta ^ { i } \sqrt [ 5 ] { b + \sqrt { b ^ { 2 } - a ^ { 5 } } } + \zeta ^ { 5 - i } \sqrt [ 5 ] { b - \sqrt { b ^ { 2 } - a ^ { 5 } } } .
$$  

证明 $\alpha _ { 0 } , \ldots , \alpha _ { 4 }$ 给出多项式  

$$
X ^ { 5 } - 5 a X ^ { 3 } + 5 a ^ { 2 } X - 2 b
$$  

的所有根, 记入重数.  

提示 将题目中的 $\alpha _ { i }$ 写作  

$$
\alpha _ { i } = \zeta ^ { i } u + \zeta ^ { - i } v ,
$$  

其中的复数 $u , v$ 满足 $u ^ { 5 } v ^ { 5 } = a ^ { 5 }$ 而 $u ^ { 5 } + v ^ { 5 } = 2 b$ . 用这些性质直接展开乘积来验证  

$$
\prod _ { i = 0 } ^ { 4 } ( X - \alpha _ { i } ) = X ^ { 5 } - 5 a X ^ { 3 } + 5 a ^ { 2 } X - 2 b .
$$  

因此 $\alpha _ { 0 } , \ldots , \alpha _ { 4 }$ 确实给出原方程的根 (含重数). 这可谓是 Cardano 公式对 5 次方程的一种推广, 有兴趣的读者可参考 [5] 的讨论.  

3. 以 Gauss–Jordan 算法解下列线性方程组.  

$$
\left\{ \begin{array} { r c c c c } { { X _ { 1 } } } & { { - 3 X _ { 2 } } } & { { - 2 X _ { 3 } } } & { { = } } & { { 3 } } \\ { { - 2 X _ { 1 } } } & { { + X _ { 2 } } } & { { - 4 X _ { 3 } } } & { { = } } & { { - 9 } } \\ { { - X _ { 1 } } } & { { + 4 X _ { 2 } } } & { { - X _ { 3 } } } & { { = } } & { { - 7 } } \end{array} \right.
$$  

$$
\left\{ \begin{array} { r c c c c } { { X _ { 1 } } } & { { + 3 X _ { 2 } } } & { { + 2 X _ { 3 } } } & { { = } } & { { 1 } } \\ { { } } & { { } } & { { } } & { { } } \\ { { 2 X _ { 1 } } } & { { + 5 X _ { 2 } } } & { { + 5 X _ { 3 } } } & { { = } } & { { 7 } } \\ { { } } & { { } } & { { } } & { { } } \\ { { 3 X _ { 1 } } } & { { + 7 X _ { 2 } } } & { { + X _ { 3 } } } & { { = } } & { { - 8 } } \\ { { } } & { { } } & { { } } & { { } } \\ { { - X _ { 1 } } } & { { - 4 X _ { 2 } } } & { { + X _ { 3 } } } & { { = } } & { { 1 0 } } \end{array} \right.
$$  

$$
\left\{ \begin{array} { r r r r r } { { X _ { 1 } } } & { { - 3 X _ { 2 } } } & { { - 2 X _ { 3 } } } & { { - X _ { 4 } } } & { { = } } & { { 6 } } \\ { { 3 X _ { 1 } } } & { { - 8 X _ { 2 } } } & { { + X _ { 3 } } } & { { + 5 X _ { 4 } } } & { { = } } & { { 0 } } \\ { { - 2 X _ { 1 } } } & { { + X _ { 2 } } } & { { - 4 X _ { 3 } } } & { { + X _ { 4 } } } & { { = } } & { { - 1 2 } } \\ { { - X _ { 1 } } } & { { + 4 X _ { 2 } } } & { { - X _ { 3 } } } & { { - 3 X _ { 4 } } } & { { = } } & { { 2 } } \end{array} \right.
$$  

(iv)  

$$
\left\{ \begin{array} { r c r c l } { { X _ { 1 } } } & { { + 3 X _ { 2 } } } & { { - 7 X _ { 3 } } } & { { = } } & { { - 8 } } \\ { { } } & { { } } & { { } } & { { } } & { { } } \\ { { 2 X _ { 1 } } } & { { + 5 X _ { 2 } } } & { { + 4 X _ { 3 } } } & { { = } } & { { 4 } } \\ { { - 3 X _ { 1 } } } & { { - 7 X _ { 2 } } } & { { - 2 X _ { 3 } } } & { { = } } & { { - 3 } } \\ { { } } & { { } } & { { } } & { { } } & { { } } \\ { { X _ { 1 } } } & { { + 4 X _ { 2 } } } & { { - 1 2 X _ { 3 } } } & { { = } } & { { - 1 5 } } \end{array} \right.
$$  

$$
\left\{ \begin{array} { r c c c c c c } { { X _ { 1 } } } & { { - 2 X _ { 2 } } } & { { + 3 X _ { 3 } } } & { { - 4 X _ { 4 } } } & { { = } } & { { 4 } } \\ { { X _ { 1 } } } & { { + X _ { 2 } } } & { { - X _ { 3 } } } & { { + X _ { 4 } } } & { { = } } & { { - 1 1 } } \\ { { X _ { 1 } } } & { { + 3 X _ { 2 } } } & { { } } & { { + X _ { 4 } } } & { { = } } & { { 1 } } \\ { { } } & { { - 7 X _ { 2 } } } & { { + 3 X _ { 3 } } } & { { + X _ { 4 } } } & { { = } } & { { - 3 } } \end{array} \right.
$$  

(v)  

4. 确定关于复数 $\mathbf { \alpha } _ { a , b }$ 的条件, 使得以下方程组有解, 并具体将解用 $a$ 和 $b$ 来表达.  

$$
\left\{ \begin{array} { r c l } { { a X _ { 1 } } } & { { + X _ { 2 } } } & { { + X _ { 3 } } } & { { = } } \\ { { X _ { 1 } } } & { { + b X _ { 2 } } } & { { + X _ { 3 } } } & { { = } } \\ { { X _ { 1 } } } & { { + 2 b X _ { 2 } } } & { { + X _ { 3 } } } & { { = } } \end{array} \right. \ 6
$$  

5. 考虑两个大小相同的矩阵  

$$
A = \left( { \begin{array} { c c c } { a _ { 1 1 } } & { \cdots } & { a _ { 1 n } } \\ { \vdots } & { \ddots } & { \vdots } \\ { a _ { m 1 } } & { \cdots } & { a _ { m n } } \end{array} } \right) , \quad B = \left( { \begin{array} { c c c } { b _ { 1 1 } } & { \cdots } & { b _ { 1 n } } \\ { \vdots } & { \ddots } & { \vdots } \\ { b _ { m 1 } } & { \cdots } & { b _ { m n } } \end{array} } \right) .
$$  

如果可以通过一系列初等行变换从 $A$ 过渡到 $B$ , 则称 $A$ 和 $B$ 是行等价的.  

(i) 取一列整数 $1 \leq c _ { 1 } < \cdot \cdot \cdot < c _ { h } \leq n$ . 从 $A$ (或 $B$ ) 删除第 $c _ { 1 } , \ldots , c _ { h }$ 列得到的矩阵记为 $\boldsymbol { A ^ { \prime } }$ (或 $B ^ { \prime }$ ). 说明若 $A$ 和 $B$ 行等价, 则 $\pmb { A } ^ { \prime }$ 和 $B ^ { \prime }$ 行等价.(ii) 设所论矩阵形如  

$$
A = \left( \begin{array} { c c c c } { { 1 } } & { { } } & { { } } & { { x _ { 1 } } } \\ { { } } & { { \ddots } } & { { } } & { { \vdots } } \\ { { } } & { { } } & { { 1 } } & { { x _ { m } } } \\ { { } } & { { } } & { { } } & { { } } \\ { { } } & { { } } & { { } } & { { } } \end{array} \right) , \quad B = \left( \begin{array} { c c c c } { { 1 } } & { { } } & { { } } & { { y _ { 1 } } } \\ { { } } & { { \ddots } } & { { } } & { { \vdots } } \\ { { } } & { { } } & { { 1 } } & { { y _ { m } } } \\ { { } } & { { } } & { { } } & { { } } \\ { { } } & { { } } & { { } } & { { } } \end{array} \right) ,
$$  

其中 $x _ { 1 } , \ldots , x _ { m }$ 和 $y _ { 1 } , \ldots , y _ { m }$ 是给定的数, 而矩阵中留白部分为 0. 说明若 $A$ 和 $B$ 行等价, 则对所有 $1 \leq i \leq m$ 皆有 $x _ { i } = y _ { i }$ .  

提示 将 $A$ 和 $B$ 视为 $\mid m \mid$ 元线性方程组的增广矩阵, 解之.  

(iii) 对于一般情形, 证明若 $A$ 和 $B$ 是行等价的简化行梯矩阵, 则 $A = B$ 提示 设 $A \neq B$ . 从左而右比较每一列, 设第一个相异的列为第 $j$ 列. 从 $A$ 和 $B$ 删除所有 $j$ 之后的列, 同时也删除第 $j$ 列之前所有不含主元的列, 得到的矩阵分别记为 $\boldsymbol { A ^ { \prime } }$ 和 $B ^ { \prime }$ .  

$\star$ 论证第 $j$ 列不可能包含主元.  
$\star$ 论证 $\pmb { A } ^ { \prime }$ 和 $B ^ { \prime }$ 必然是 (ii) 之中的形式. 配合 (i) 来推导 $\pmb { A } ^ { \prime } = \pmb { B } ^ { \prime }$ , 从而导出矛盾.  

6. (Leontief 投入–产出模型) 考虑一个理想化的经济体, 它有 $\mathbf { \Omega } _ { n }$ 个生产部门, 部门 $\mathbf { \chi } _ { i }$ 只生产类型 $\mathbf { \chi } _ { i }$ 的产品, 而生产过程投入的要素遵循固定的比例. 将这些产品统一以元计价. 设部门 $i$ 产出价值 $X _ { i }$ 元的产品, 而部门 $j$ 每生产 1 元的产品需要投入 $a _ { i j }$ 元的第 $\mathbf { \chi } _ { i }$ 种产品, 其中$1 \leq i , j \leq n$ . 因此价值 $X _ { i }$ 元的类型 $\mathbf { \chi } _ { i }$ 产品一部分供给其他部门 (称为中间产品需求), 剩下部分则供给消费者 (称为最终产品需求), 记后一部分的价值为 $d _ { i } \in \mathbb { R } _ { \geq 0 }$ . 列式得到  

$$
X _ { i } = a _ { i 1 } X _ { 1 } + \cdot \cdot \cdot + a _ { i n } X _ { n } + d _ { i } .
$$  

让 $1 \leq i \leq n$ 变动便得到 $\mathbf { \Omega } _ { n }$ 元线性方程组, 其中的系数 $a _ { i j } \in \mathbb { R } _ { \geq 0 }$ 称为投入系数. 将这些系数作成 $n \times n$ 矩阵  

$$
A = \left( \begin{array} { c c c } { { a _ { 1 1 } } } & { { \cdot \cdot \cdot } } & { { a _ { 1 n } } } \\ { { } } & { { } } & { { } } \\ { { \vdots } } & { { } } & { { \vdots } } \\ { { } } & { { } } & { { } } \\ { { a _ { n 1 } } } & { { \cdot \cdot \cdot } } & { { a _ { n n } } } \end{array} \right) .
$$  

说明之前的线性方程组能以增广矩阵的写法表作  

$$
\left( \left. \mathbf { 1 } _ { n \times n } - \mathbf { A } \right| d \right) ,
$$  

其中 ${ \mathbf { 1 } } _ { n \times n }$ 是对角线上为 1, 其余位置全为 0 的 $n \times n$ 矩阵, 称为单位矩阵, $\mathbf { 1 } _ { n \times n } - A$ 意谓将两个矩阵逐项相减 (请写出它的大致样貌), 而 $\textbf { \em d }$ 是只有一列的 $n \times 1$ 矩阵 (常称为 $\mathbf { \Omega } _ { n }$ 维列向量)  

$$
\begin{array} { r } { \pmb { d } = \left( \begin{array} { c } { d _ { 1 } } \\ { . } \\ { . } \\ { d _ { n } } \end{array} \right) . } \end{array}
$$  

针对投入–产出的初步分析化为线性方程组或对应的矩阵的研究. 并非所有解都有现实意义; 比方说, 我们希望在所有 $\mathbf { \Phi } _ { d _ { i } }$ 非负的前提下, 总存在使所有 $X _ { i }$ 非负的解. 为了确定是否恒有这种解2), 同时在 $\boldsymbol { n }$ 较大时高效地计算, 需要对矩阵有更加透彻的了解, 一句话, 需要更高段位的数学.  